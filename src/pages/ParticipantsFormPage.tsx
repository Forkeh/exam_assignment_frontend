...
import { createParticipant, getParticipantById, updateParticipant } from "@/services/ParticipantApi";
...
export default function ParticipantsFormPage() {
	...
	useEffect(() => {
		...
		getParticipantById(participant.id!)
			...
			.catch(() => {
				toast({
					title: "Oh no! Something went wrong!",
					description: `We could not fetch the participant from the server. Please try again later.`,
					variant: "destructive",
				});
			});
	}, [participant]);
	...
	return (
		<>
			<h2 className="mb-5 text-pretty text-center text-3xl font-bold sm:text-5xl">{participant ? "Update" : "Create"} Participant</h2>
